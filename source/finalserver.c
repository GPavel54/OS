...
void * threadForUser(void * arg){
  char menu[150], again[150] = "again\n";
  char sendOk[150] = "ok\n";
  char secondP[150] = "You are the second player. Waiting for first player move.\n";
  char nextTime[150] = "Cannot create more games. Try later.";
  ...
  while (check == 0){
    ...
    if (answer > myInfo->maxgames){
      send(myInfo->sockfd, again, 150, 0);
      continue;
    }
    ...
    if (myInfo->games[answer - 1][1] != 0 && myInfo->games[answer - 1][2] == 0){
      ...
      send(myInfo->sockfd, sendOk, 150, 0);
      return NULL;
    }
  }
  int i = 0;
  ...
  if (i == myInfo->maxgames){
    send(myInfo->sockfd, nextTime, 150, 0);
    return NULL;
  }
  send(myInfo->sockfd, sendOk, 150, 0);
  myInfo->games[i][0] = 1;
  myInfo->games[i][1] = myInfo->sockfd;
  char first[150] = "You are the first player. Waiting 60 seconds for second.\n";
  send(myInfo->sockfd, first, 150, 0);
  ...
  struct board b = {{0}};
  send(myInfo->games[i][2], secondP, 150, 0);
  int win = 0;
  while (win == 0){
